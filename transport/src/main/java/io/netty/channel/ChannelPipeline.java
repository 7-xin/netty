 ...
 *   {@link ChannelPipeline} implements an advanced form of the <a href="https://www.oracle.com/technetwork/java/interceptingfilter-142169.html">Intercepting Filter</a> pattern
 * to give a user full control over how an event is handled and how the {@link ChannelHandler}s in a pipeline interact with each other.
 *
 ...
 *
 *   The following diagram describes how I/O events are processed by {@link ChannelHandler}s in a {@link ChannelPipeline} typically.
 *   An I/O event is handled by either a {@link ChannelInboundHandler} or a {@link ChannelOutboundHandler} and be forwarded to its closest handler by calling the event propagation methods defined in {@link ChannelHandlerContext},
 * such as {@link ChannelHandlerContext#fireChannelRead(Object)} and {@link ChannelHandlerContext#write(Object)}.
 *
 * <pre>
 *                                                 I/O Request
 *                                            via {@link Channel} or
 ...
 * The inbound data is often read from a remote peer via the actual input operation such as {@link SocketChannel#read(ByteBuffer)}.
 ...
 * If an outbound event goes beyond the bottom outbound handler, it is handled by an I/O thread associated with the {@link Channel}.
 * The I/O thread often performs the actual output operation such as {@link SocketChannel#write(ByteBuffer)}.
 * <p>
 *
 ...
 * When an event goes outbound, the order is 5, 4, 3, 2, 1.  On top of this principle, {@link ChannelPipeline} skips the evaluation of certain handlers to shorten the stack depth:
 * <ul>
 *  <li>3 and 4 don't implement {@link ChannelInboundHandler}, and therefore the actual evaluation order of an inbound event will be: 1, 2, and 5.</li>
 *  <li>1 and 2 don't implement {@link ChannelOutboundHandler}, and therefore the actual evaluation order of a outbound event will be: 5, 4, and 3.</li>
 *  <li>If 5 implements both {@link ChannelInboundHandler} and {@link ChannelOutboundHandler}, the evaluation order of an inbound and a outbound event could be 125 and 543 respectively.</li>
 * </ul>
 *
 * <h3>Forwarding an event to the next handler</h3>
 * todo 转发到下一个处理器
 *
 * As you might noticed in the diagram shows, a handler has to invoke the event propagation methods in {@link ChannelHandlerContext} to forward an event to its next handler.
 ...
 * todo 构建 pipeline
 * <p>
 * A user is supposed to have one or more {@link ChannelHandler}s in a pipeline to receive I/O events (e.g. read) and to request I/O operations (e.g. write and close).
 * For example,
 * a typical server will have the following handlers in each channel's pipeline, but your mileage may vary depending on the complexity and characteristics of the protocol and business logic:
 *
 * <ol>
 ...
 * </pre>
 *
 * Be aware that while using {@link DefaultEventLoopGroup} will offload the operation from the {@link EventLoop} it will still process tasks in a serial fashion per {@link ChannelHandlerContext} and so guarantee ordering.
 * Due the ordering it may still become a bottle-neck.
 * If ordering is not a requirement for your use-case you may want to consider using {@link UnorderedThreadPoolEventExecutor} to maximize the parallelism of the task execution.
 *
 * <h3>Thread safety</h3>
 * todo 线程安全的
 * <p>
...
public interface ChannelPipeline extends ChannelInboundInvoker, ChannelOutboundInvoker, Iterable<Entry<String, ChannelHandler>> {
     ...
     *
     * @return the handler with the specified name. {@code null} if there's no such handler in this pipeline.
     */
     ...
     *
     * @return the handler of the specified handler type. {@code null} if there's no such handler in this pipeline.
     */
     ...
     *
     * @return the context object of the specified handler. {@code null} if there's no such handler in this pipeline.
     */
     ...
     *
     * @return the context object of the handler with the specified name. {@code null} if there's no such handler in this pipeline.
     */
     ...
     *
     * @return the context object of the handler of the specified type. {@code null} if there's no such handler in this pipeline.
     */
    ...
    /**
     * Converts this pipeline into an ordered {@link Map} whose keys are handler names and whose values are handlers.
     */
    Map<String, ChannelHandler> toMap();

    // todo ------------------------ ChannelInboundInvoker 相关 ------------------------
    @Override
    ...
    // todo ------------------------ ChannelOutboundInvoker 相关 ------------------------
    @Override
