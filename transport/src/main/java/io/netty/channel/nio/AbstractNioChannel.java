    ...
    protected void doRegister() throws Exception {
        ...
        for (;;) {
            try {
                 ...
                 * todo     于是selector就在这里提前把channel存放入 attachment中, 后来使用
                ...
                if (!selected) {
                    // Force the Selector to select now as the "canceled" SelectionKey may still be cached and not removed because no Select.select(..) operation was called yet.
                    eventLoop().selectNow();
                    selected = true;
                } else {
